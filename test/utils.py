...
def send_data_batch_worker(api_url, send_interval=3):
    ...
    """
    while True:
        # # Chờ 'send_interval' giây trước khi gửi dữ liệu (tránh gửi liên tục, giảm tải server)
        time.sleep(send_interval)
        # # Dùng lock để tránh xung đột khi truy cập 'send_buffer' từ nhiều thread
        with send_lock:
            buffer_copy = send_buffer.copy() # Sao chép dữ liệu cần gửi
            send_buffer.clear() # Xóa dữ liệu gốc để chuẩn bị nhận dữ liệu mới
        for record in buffer_copy:
            try:
                # # Gửi dữ liệu dạng JSON tới API qua phương thức POST
...
def is_real_face(frame, bbox, image_cropper, model_test, model_dir, model_name, scale, w_input, h_input):
    ...
    param = {
        ...
        "out_w": w_input,
        "out_h": h_input,
        "crop": True if scale is not None else False, # Gán crop = True nếu scale có giá trị, ngược lại crop = False
    }
    # Crop ảnh khuôn mặt từ bbox, đồng thời resize về (w_input, h_input) lấy từ parse_model_name về 80 * 80
