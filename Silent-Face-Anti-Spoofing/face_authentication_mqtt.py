...
# mqtt_client.connect(MQTT_BROKER, MQTT_PORT, 60)
# mqtt_client.loop_start()  # Chạy vòng lặp MQTT nền
...
# # ====== Send batch thread to send HTTP POST requests ======
# send_buffer = []
# send_lock = threading.Lock()
# send_interval = 3
...
# last_fake_upload_time = 0
# fake_upload_interval = 10  # seconds
...
#                     print(f"Send MQTT: {payload}")
#                 else:
#                     print(f"Failed to send MQTT message, rc={result.rc}")
#         else:
#             label_text = f"Fake Face ({confidence:.1f}%)"
#             color = (0, 0, 255)
...
# cap.release()
# cv2.destroyAllWindows()
# fake_face_queue.put((None, None))  # signal thread to end
